...
pdf("advance_zoom2.pdf", 8, 8);
par(mar=c(2, 2, 2, 2));

plot(c(1,800), c(1,800), type="n", axes=FALSE, xlab="", ylab="", main="advance_zoom2.R");

angle.start <- 0;
angle       <- 110;

for (i in 1:3){
  angle.end <- angle.start + angle;
  chr.n     <- sample(3:8, 1);
  chr.i     <- sample(1:22, chr.n);
  chr.i     <- sort(chr.i);
  ...
  circos(R=290, cir=chr.db, W=100, mapping=expr,  col.v=60,  type="heatmap2", 
